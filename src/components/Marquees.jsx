...
const Marquees = () => {
    ...
    return(
        <div className="bg-zinc-900 py-20 w-full relative overflow-hidden " style={{ backgroundimage: 'radial-gradient(circle farthest-corner at 50% 50%, transparent, black 50%)', minwidth: '200vw', height: '100%'}}>
            {images.map((item,index)=><Marquee key={index} direction={index===0?"left":"right"} imgurl={item} />)}
        </div>
    )
}
