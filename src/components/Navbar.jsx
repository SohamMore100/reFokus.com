      ...
      <div className='n-left flex items-center'>
        <div className="links flex gap-14 ml-2">
          ...
          {["Home","Work","Culture","","News"].map((elem,index) => (
            elem.length ===0?<span key={index} className='w-[2px] h-8 bg-zinc-600'></span> :
            <a key={index}  className='font-regular text-md flex items-center gap-2 hover:text-gray-500'  href='#'>
            
              {index === 0 && <span key={index}  style={{boxShadow:"0 0 0.46em #00FF19"}} className='inline-block w-1.5 h-1.5 rounded-full bg-green-500'></span>}
              {elem}
            </a>))
          }
        </div>
